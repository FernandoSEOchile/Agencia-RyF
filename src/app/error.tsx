...
export default function Fallo({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  return (
    <main className="mx-auto flex min-h-[70vh] max-w-md flex-col items-center justify-center px-6 text-center">
      ...
      <h1 className="mt-2 text-[24px] font-bold">Esta pantalla no se pudo cargar</h1>
      ...
      {error.digest && (
        <p className="mt-2 font-mono text-[12px] text-[color:var(--tinta-suave)]">ref. {error.digest}</p>
      )}
