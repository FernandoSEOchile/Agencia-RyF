...
// La paleta sale de la marca y no de un grafico de ejemplo: el naranja es el
// del panel, el azul es profundo para no parecer un enlace, y las lineas de
// la rejilla son un pelo mas claras que las de las tarjetas.
const AZUL = "#2e63a6";
const NARANJA = "#ff6b00";
const VERDE = "#1f9d6a";
const AMARILLO = "#e3a008";
const TINTA = "#1d1d1f";
const SUAVE = "#8e8e93";
const LINEA = "#ececf1";
  ...
  if (puntos.length < 2) {
    return (
      <div className="tarjeta p-5">
        ...
        <p className="mt-6 text-center text-[14px] text-[color:var(--tinta-suave)]">
  ...
  return (
    <div className="tarjeta p-5">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <h3 className="text-[14px] font-medium">{titulo}</h3>
        <p className="text-[14px] tabular-nums text-[color:var(--tinta-media)]">
          {marcado ? (
            <>
      ...
      >
        ...
        <text x={PAD.izquierda} y={alto - 8} fontSize="12" fill={SUAVE}>
          {dia(puntos[0].fecha)}
        </text>
        <text x={ANCHO - PAD.derecha} y={alto - 8} fontSize="12" fill={SUAVE} textAnchor="end">
          {dia(ultimo.fecha)}
        </text>
      </svg>
    </div>
...
export function Tramos({ dias }: { dias: DiaPosiciones[] }) {
  if (dias.length === 0) {
    return (
      <div className="tarjeta p-5">
        <h3 className="text-[14px] font-medium">Palabras por tramo</h3>
        <p className="mt-6 text-center text-[14px] text-[color:var(--tinta-suave)]">
    ...
    <div className="tarjeta p-5">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          {TRAMOS.map((t) => (
            <span key={t.id} className="flex items-center gap-1.5 text-[13px] text-[color:var(--tinta-media)]">
      ...
      <svg viewBox={`0 0 ${ANCHO} ${ALTO}`} className="mt-2 w-full" style={{ height: ALTO }} role="img"
        ...
        <text x={PAD.lados} y={ALTO - 8} fontSize="12" fill={SUAVE}>{dia(dias[0].fecha)}</text>
        <text x={ANCHO - PAD.lados} y={ALTO - 8} fontSize="12" fill={SUAVE} textAnchor="end">{dia(ultimo.fecha)}</text>
      </svg>
    </div>
  );
}
...
export function Historico({ meses }: { meses: MesTramos[] }) {
  ...
  return (
    <div className="tarjeta p-5">
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <h3 className="text-[14px] font-medium">Palabras clave por posición</h3>
        <p className="text-[14px] tabular-nums text-[color:var(--tinta-media)]">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1">
        {RAMPA.map((r) => (
          <span key={r.id} className="flex items-center gap-1.5 text-[13px] text-[color:var(--tinta-media)]">
      ...
      >
        ...
        <text x={PAD.lados} y={ALTO - 8} fontSize="12" fill={SUAVE}>{mesCorto(meses[0].mes)}</text>
        <text x={ANCHO - PAD.lados} y={ALTO - 8} fontSize="12" fill={SUAVE} textAnchor="end">{mesCorto(ultimo.mes)}</text>
      </svg>
    </div>
  );
}
        ...
        {[0, 1, 2, 3, 4].map((n) => {
          ...
          return (
            <g key={n}>
              ...
              <text x={ANCHO - PAD.derecha + 8} y={y(v) + 4} fontSize="12" fill={SUAVE}>
                {miles(v)}
              </text>
            </g>
          );
...
export function Reparto({
  ...
  return (
    <div className="tarjeta p-5">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <h3 className="text-[14px] font-medium">Posiciones reales en Google</h3>
        <p className="text-[14px] tabular-nums text-[color:var(--tinta-media)]">
      ...
      <div className="mt-4 flex flex-col gap-2.5">
        {BARRAS.map((b) => (
          <div key={b.id} className="flex items-center gap-3">
            <span className="w-16 shrink-0 text-right text-[13px] tabular-nums text-[color:var(--tinta-media)]">
              {b.etiqueta}
            </span>
            ...
            <span className="w-14 shrink-0 text-[13px] font-medium tabular-nums">
              {miles(b.valor)}
            </span>
          </div>
        ))}
...
export function Cifra({
  ...
  return (
    <div className="bg-[color:var(--panel)] px-5 py-4">
      ...
      <p className="mt-1.5 cifra text-[28px] leading-none" style={{ color: TINTA }}>
        {valor}
      </p>

      {variacion != null && variacion !== 0 && (
        <p className={`mt-1.5 text-[13px] font-medium tabular-nums ${bueno ? "text-emerald-700" : "text-red-600"}`}>
          {sube ? "▲" : "▼"} {Math.abs(variacion)}%{" "}
          <span className="font-normal text-[color:var(--tinta-suave)]">vs. periodo anterior</span>
        </p>
      )}

      {pie && <p className="mt-1.5 text-[13px] text-[color:var(--tinta-suave)]">{pie}</p>}
      {accion && (
        <button
          type="button"
          onClick={accion.alPulsar}
          className="mt-2 text-[13px] font-medium text-[color:var(--acento)] underline-offset-4 hover:underline"
        >
          {accion.texto} →
        </button>
      )}
